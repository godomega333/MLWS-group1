...
def parse_args():
    parser = argparse.ArgumentParser(description="XGBoost fixed-config comparison run.")
...
def main():
    ...
    pd.DataFrame({'PassengerId': passenger_ids, 'Transported': preds}).to_csv(
        os.path.join(args.output_dir, "submission_xgboost_comparison.csv"), index=False)
    pd.DataFrame({
        "metric": ["stratified_cv_acc"] + [f"fold{i+1}" for i in range(len(fold_accuracies))],
        "value":  [cv_acc] + fold_accuracies,
    }).to_csv(os.path.join(args.output_dir, "metrics_xgboost_comparison.csv"), index=False)

    print(f"    stratified_5fold_cv_acc = {cv_acc:.5f}")
    print(f">>> Saved submission to: {os.path.join(args.output_dir, 'submission_xgboost_comparison.csv')}")
